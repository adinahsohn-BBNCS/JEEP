# Fourbie Exchange — Featured Listing Brief

**Listing tier:** Featured ($119 one-time)  
**Marketing site:** https://goanywherejeep.com/  
**Seller:** Private party  
**Contact:** Adi@bbncs.com · 951-251-5656  
**Location:** Temecula, CA  
**Asking price:** $100,000

Use this file to submit the Featured listing and to brief Fourbie’s listing specialist. Upload photos from the **full-res** files in `pics/` (not `pics/display/` or `pics/thumbs/`).

---

## 1. Specs (form fields)

| Field | Value |
|--------|--------|
| Year | 2014 |
| Make | Jeep |
| Model | Wrangler Unlimited |
| Trim | Rubicon |
| Exterior color | Amp'd (factory yellow) |
| Interior | Tan |
| Hardtop | Color-matched hardtop |
| Odometer | 40,734 |
| Engine | Chevrolet Performance LT1 6.2L Gen V V8 (MoTech Premium Kit; MoTech tuned ~496 hp / 496 lb-ft) |
| Fuel type | Gasoline |
| Transmission | GM 8L90 8-speed automatic |
| Drivetrain | 4×4 — Advance Adapters Atlas twin-stick transfer case |
| Axle gears | 5.38 |
| Title status | Clean |
| Seller type | Private party |
| VIN / Chassis # | 1C4BJWFG2EL178933 |
| Price | $100,000 |
| Location | Temecula, CA |
| Phone | 951-251-5656 |
| Email | Adi@bbncs.com |
| Site / more photos | https://goanywherejeep.com/ |

### Gaps to fill before checkout

- **VIN:** 1C4BJWFG2EL178933 _(entered as 17-character Jeep WMI `1C4…` from your `1CBJ…` — confirm on dash/title)_
- **Exact odometer:** 40,734
- **Known imperfections / disclosures:** _______________________________  
  _(paint chips, trail scars, interior wear, mechanical notes, warning lights, etc. — list honestly; Fourbie recommends showing flaws)_

Contact confirmed as phone + Adi@bbncs.com unless you change it above.

---

## 2. Suggested listing title

**2014 Jeep Wrangler Unlimited Rubicon Amp'd — MoTech LT1 V8, Currie 60/70, EVO Double Throwdown, 40s — 40,734 mi — Temecula, CA**

Short card blurb (homepage-style):

> 40,734 mi, MoTech LT1 V8 + 8L90, Currie 60/70, EVO Double Throwdown, Atlas, 40" Mickey Thompsons, Amp'd yellow / tan

---

## 3. Ad description (paste into listing)

> **Please keep this voice and structure.** Featured includes professional copy support — use the text below as the source of truth and preserve the Amp'd branding plus a prominent link to https://goanywherejeep.com/. Do not rewrite this into a generic stock-Rubicon ad.

---

This 2014 Jeep Wrangler Unlimited Rubicon 4×4 (VIN 1C4BJWFG2EL178933) is offered by a private party in Temecula, California with 40,734 miles and an asking price of $100,000. Finished in rare factory Amp'd yellow over tan with a color-matched hardtop, this four-door Rubicon was built as a serious rock crawler — not a sticker build — and has been sitting more than it's crawling since.

At about 30,000 miles, MoTech Premium Conversions in Las Vegas installed a Chevrolet Performance LT1 6.2L Gen V V8 Premium Kit paired to a GM 8L90 eight-speed automatic. Factory-rated 455 hp / 455 lb-ft and MoTech tuned to approximately 496 hp / 496 lb-ft, with roughly 10,000 miles on the swap. The rest of the package matches the power: Currie Extreme 60 front and RockJock 70 full-float rear axles, 5.38 gears, ARB Air Lockers front and rear, EVO / Off Road Evolution Double Throwdown long-travel coilover and bypass (King), Currie AntiRock sway bars, Advance Adapters Atlas twin-stick transfer case, PSC hydraulic steering assist, and 40" Mickey Thompson mud terrains.

Full gallery, build story, and manufacturer links: **https://goanywherejeep.com/**

- **Not a sticker build.** Factory Trail Rated, then built for the rocks — axle, suspension, armor, and trail gear chosen for real use.
- **Rare OEM finish.** Factory Amp'd yellow over tan with a color-matched hardtop.
- **Professional MoTech LT1 + 8L90.** Installed at ~30k miles; ~10k miles on the powertrain swap.
- **Low-mile chassis.** 40,734 miles — sitting more than crawling.
- **Serious hardware.** Currie Extreme 60 / RockJock 70, EVO Double Throwdown with King, Atlas twin-stick, ARB lockers, 40s.
- **Trail kit included.** What you see on the Jeep goes with the sale — lights, winch, armor, spare carrier gear, compressors, audio, and the rest of the trail equipment.
- **Clean title.** Receipts available on request for MoTech powertrain, EVO Double Throwdown suspension, and Currie Extreme 60 / RockJock 70 axle builds.
- **Drive it.** Hit the rocks, fill the tank, drive it home — dialed on-road and off.

### Powertrain

- Chevrolet Performance LT1 crate — 6.2L Gen V V8
- Factory rating: 455 hp / 455 lb-ft
- MoTech tuned: approximately 496 hp / 496 lb-ft
- GM 8L90 8-speed automatic
- MoTech Premium Conversions install (Las Vegas) at ~30,000 miles
- Roughly 10,000 miles on the swap
- Advance Adapters Atlas twin-stick transfer case
- PSC hydraulic steering assist (for 40s)

### Chassis and driveline

- Currie Extreme 60 front axle
- Currie RockJock 70 full-float rear axle
- 5.38 axle gears
- ARB Air Lockers front and rear
- Currie AntiRock sway bars front and rear

### Suspension and tires

- EVO / Off Road Evolution Double Throwdown long-travel coilover + bypass
- King coilovers / racing shocks
- 40" Mickey Thompson mud terrains
- Full-size spare on carrier

### Armor, recovery, and trail gear

- Poison Spyder fenders, sliders & body armor
- GenRight front & rear bumpers
- Rock Hard 4x4 roll cage
- Rock-Slide Engineering steps
- Warn winch with wireless controller
- ARB dual onboard compressor
- Premier Power Welder onboard welding system
- Spare carrier gear (including RotopaX and high-lift jack as shown)

### Lighting, cabin, and audio

- Baja Designs LEDs — A-pillar, windshield, bumper, and magnetic rock lights
- sPOD accessory control
- Alpine head unit
- JL Audio speakers throughout
- Atlas twin-stick layout for low-range work

### Finish

- Factory Amp'd yellow exterior
- Tan interior
- Color-matched hardtop

### Included with sale

Everything on the Jeep goes with the sale — lights, winch, armor, spare carrier gear, compressors, audio, and the rest of the trail equipment. No parting out the build.

### Ownership and records

- Clean title
- Receipts on request: MoTech LT1 V8 Premium Kit & 8-speed install; EVO / Off Road Evolution Double Throwdown; Currie Extreme 60 / RockJock 70
- Private party sale — Temecula, CA

**See the full story and photo gallery:** https://goanywherejeep.com/

Inquire by email at Adi@bbncs.com or call/text 951-251-5656.

---

## 4. Photo upload order

Upload from `pics/` full-resolution files (28 images). Suggested storytelling order for Fourbie:

| # | File | What it shows |
|---|------|----------------|
| 1 | `pics/19-rocks.jpg` | Trail-proven rock crawl / flex |
| 2 | `pics/20-trail.jpg` | Rocky trail climb |
| 3 | `pics/21-carcrush.jpg` | Event / capability shot |
| 4 | `pics/5-DSC02056.jpg` | Front three-quarter hero (site hero) |
| 5 | `pics/6-DSC02061.jpg` | Front three-quarter alternate |
| 6 | `pics/15-DSC02059.jpg` | Low front three-quarter |
| 7 | `pics/28-DSC02057.jpg` | Three-quarter night stance |
| 8 | `pics/25-DSC02048.jpg` | Full side profile with rock lights |
| 9 | `pics/26-DSC02050.jpg` | Full side profile alternate |
| 10 | `pics/27-DSC02049.jpg` | Three-quarter with headlights + rock lights |
| 11 | `pics/7-DSC02068.jpg` | Low front / Warn winch stance |
| 12 | `pics/8-DSC02090.jpg` | Head-on front |
| 13 | `pics/16-DSC02072.jpg` | Head-on front with Baja Designs + winch |
| 14 | `pics/9-DSC02114.jpg` | Rear / spare carrier / RotopaX |
| 15 | `pics/10-DSC02116.jpg` | Head-on rear with spare, RotopaX, high-lift |
| 16 | `pics/11-DSC02125.jpg` | Rear three-quarter |
| 17 | `pics/13-DSC01987.jpg` | Front suspension, King, PSC, winch |
| 18 | `pics/17-DSC02108.jpg` | King shocks / AntiRock detail |
| 19 | `pics/23-DSC02112.jpg` | Currie AntiRock close-up |
| 20 | `pics/22-DSC01977.jpg` | Rear Currie axle / bumper undercarriage |
| 21 | `pics/24-DSC02082.jpg` | Poison Spyder armor detail |
| 22 | `pics/1-DSC01995.jpg` | Engine bay (PSC reservoir / intake) |
| 23 | `pics/18-20260711_212951.jpg` | LT1 bay with Corvette cover / Spectre intake |
| 24 | `pics/14-DSC02018.jpg` | Full cockpit / cage / Alpine / RAM mount |
| 25 | `pics/2-DSC02014.jpg` | Driver seating position |
| 26 | `pics/4-DSC02045.jpg` | Alpine + sPOD center stack |
| 27 | `pics/29-DSC02047.jpg` | Cabin close-up / twin-stick shifters |
| 28 | `pics/3-DSC02039.jpg` | Transmission + Atlas shifter detail |

**Also share with specialist (reference, not required as listing #1):**  
https://goanywherejeep.com/og-image.jpg · full site https://goanywherejeep.com/

**Photo credit (optional on listing):** [@shootingupthestreets](https://www.instagram.com/shootingupthestreets/)

### Gaps vs Fourbie Photo & Video Guide

Your pro set covers exterior, trail use, engine, cabin, armor, and some undercarriage. Before or after Featured submit, consider adding:

| Missing / weak | Why Fourbie wants it |
|----------------|----------------------|
| Gauge cluster with **odometer clearly visible** | Confirms mileage |
| Additional undercarriage angles (frame rails, front axle, transfer case, skids) | Builds buyer trust on a 4×4 |
| Daylight driver-side / passenger-side straight profiles | Complements night hero set |
| Documentation photos (keys, title redacted, MoTech / EVO / Currie receipts) | Supports the “receipts on request” claim |
| Known **flaw / wear** close-ups | Filters unserious buyers; required honesty |
| Optional short videos: walkaround, cold start, idle, brief drive, lockers/lights/winch demo | Strong for Featured listings |

---

## 5. Notes for Fourbie Featured specialist

Please treat this as a **documented custom rock crawler**, not a lightly modified stock Rubicon.

1. Preserve **Amp'd** branding and the marketing site URL **https://goanywherejeep.com/** in the published ad (hero/media link and/or description).
2. Prefer keeping the seller-provided description voice above; polish for clarity if needed, but do not strip the MoTech / Currie / EVO / Atlas selling points or reframe as a stock JK.
3. Headline emphasis should lead with Amp'd + MoTech LT1 + Currie 60/70 + 40s, not generic “lifted Rubicon.”
4. Inquiries: **Adi@bbncs.com** and **951-251-5656**.
5. Receipts available for MoTech powertrain, EVO Double Throwdown, and Currie Extreme 60 / RockJock 70.
6. Photos by @shootingupthestreets — credit optional.
7. Seller can provide additional undercarriage, odometer, receipt, and flaw photos on request.

---

## 6. How to submit the Featured listing

1. Confirm **VIN** on dash/title matches `1C4BJWFG2EL178933`, and add **flaw notes** in section 1 if any.
2. Open [Fourbie Sell / Classified Options](https://www.fourbieexchange.com/sell) → **Choose Featured** ($119 one-time).
3. Create account / sign in and complete checkout.
4. Enter vehicle fields from the specs table (section 1) — odometer **40,734**, VIN **1C4BJWFG2EL178933**.
5. Paste the ad description (section 3) into the listing body.
6. Upload the 28 full-res photos from `pics/` in the order in section 4.
7. Set contact to Adi@bbncs.com / 951-251-5656.
8. Immediately email (or message your assigned specialist) using the template below so they keep your site branding instead of a generic rewrite.
9. Optional: send any new odometer / undercarriage / receipt / flaw photos and a short landscape walkaround or cold-start video.

### Email template to Fourbie

**To:** info@fourbieexchange.com *(or your assigned Featured listing specialist)*  
**Subject:** Featured listing — 2014 Amp'd Rubicon — please keep site copy & goanywherejeep.com

```
Hi Fourbie team,

I just submitted / am submitting a Featured listing for my 2014 Jeep Wrangler Unlimited Rubicon (Amp'd) in Temecula, CA — $100,000.

I already built a full marketing site for this Jeep and would like the Featured ad to incorporate that branding and copy rather than a generic rewrite:

https://goanywherejeep.com/

Attached / pasted below is my listing brief with:
- Specs and contact
- Preferred title and full description (please keep this voice)
- Photo upload order
- Notes for the listing specialist

Key points for buyers:
- Factory Amp'd yellow / tan
- MoTech LT1 V8 Premium Kit + GM 8L90 (~496 hp tuned), installed at ~30k miles
- Currie Extreme 60 / RockJock 70, EVO Double Throwdown, Atlas, ARB lockers, 40" Mickey Thompsons
- 40,734 miles, clean title, VIN 1C4BJWFG2EL178933
- Everything on the Jeep is included
- Inquiries: Adi@bbncs.com / 951-251-5656

Please keep the Amp'd branding and a prominent link to https://goanywherejeep.com/ on the published listing.

Thank you,
Adi
951-251-5656
Adi@bbncs.com
```

Attach this file (`fourbie-listing-brief.md`) or paste sections 1–5 into the email body.

---

## 7. Quick checklist

- [x] Insert VIN — 1C4BJWFG2EL178933 (confirm on vehicle)
- [x] Insert exact odometer — 40,734
- [ ] List known imperfections
- [ ] Confirm contact still Adi@bbncs.com / 951-251-5656
- [ ] Choose Featured ($119) at https://www.fourbieexchange.com/sell
- [ ] Paste description + upload `pics/` in order
- [ ] Email specialist with this brief + site URL
- [ ] (Recommended) Add odometer cluster photo, more undercarriage, receipts, flaw shots, short video
